import { runtimePayouts } from '$lib/constants/game';
import {
  rowCount,
  winRecords,
  riskLevel,
...
class PlinkoEngine {
  ...
  dropBall() {
    ...
    const ball = Matter.Bodies.circle(
      ...
      ballRadius,
      {
        restitution: get(bounciness), // Bounciness
        friction,
  ...
  private handleBallEnterBin(ball: Matter.Body) {
    ...
    if (binIndex !== -1 && binIndex < this.pinsLastRowXCoords.length - 1) {
      ...
      const multiplier = get(runtimePayouts)[this.rowCount][this.riskLevel][binIndex];
